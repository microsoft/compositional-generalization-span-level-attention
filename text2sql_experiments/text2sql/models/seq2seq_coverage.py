...
class AttentionCoverageSeq2Seq(Model):
    ...
    def forward(
        ...
        target_to_source_alignment: torch.LongTensor = None,
        ...
        if target_tokens:
            ...
            # and validation.
            output_dict = self._forward_loop(state, target_tokens, target_to_source_alignment)
        else:
            output_dict = {}
        ...
        if not self.training:
            ...
            if target_tokens:
                ...
                if self._token_based_metric:
                    ...
                    for idx, (hyp, tgt) in enumerate(zip(predicted_tokens, target_tokens_str)):
                        original_tgt = metadata[idx]['target_tokens']
                        if (hyp == tgt) != (hyp == original_tgt):
                            print('Hyp:', predicted_tokens)
                            print('Tgt:', tgt)
                            print('Original Tgt:', original_tgt)
                            raise RuntimeError
                        is_correct = hyp == tgt
                        metadata[idx]['is_prediction_correct'] = is_correct

                        # self.eval_list.append(is_correct)

                self._kb_match(predicted_tokens, target_tokens_str)
                self._schema_free_match(predicted_tokens, target_tokens_str)

        if 'attention_input_weights' in output_dict:
            del output_dict['attention_input_weights']

        output_dict['metadata'] = metadata

        # print(f'Cumulative acc: {np.average(self.eval_list):.5f} on {len(self.eval_list)} samples')
    ...
    def _forward_loop(self,
                      state: Dict[str, torch.Tensor],
                      target_tokens: Dict[str, torch.LongTensor] = None,
                      target_to_source_alignment: torch.Tensor = None) -> Dict[str, torch.Tensor]:
        """
        ...
        if target_tokens:
            ...
            if self._attention_regularization and self.training:
                assert self._attention_regularization.startswith('mse')
                ...
                # shape: (batch_size, target_sequence_length - 1, source_sequence_length)
                att_reg = torch.nn.MSELoss(reduction='none')(
                    attention_weights, target_to_source_alignment[:, 1:]
                ) * target_to_source_alignment_mask[:, 1:]

                # shape: (batch_size, target_sequence_length - 1)
                # num_attn_regularized_src_tokens = state['source_tokens_mask'].sum(dim=-1, keepdim=True)
                # shape: (batch_size, )
                num_attn_regularized_tgt_tokens = (target_to_source_alignment[:, 1:].sum(dim=-1) > 0).sum(dim=-1)
                ...
                att_reg = (
                      ...
                      num_attn_regularized_tgt_tokens + 1e-13
                  )

                # shape: scalar
                att_reg_loss = att_reg.mean() * self._attention_reg_weight

            output_dict["loss"] = loss + self._coverage_lambda * coverage_loss + att_reg_loss
