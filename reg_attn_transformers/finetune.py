...
class SummarizationModule(BaseTransformer):
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument(
            "--val_metric", type=str, default=None, required=False, choices=["bleu", "rouge2", "loss", "acc", None]
        )
        parser.add_argument("--eval_max_gen_length", type=int, default=None, help="never generate more than n tokens")
...
class TranslationModule(SummarizationModule):
    ...
    def _generative_step(self, batch: dict) -> dict:
        ...
        for idx in range(len(input_ids_array)):
            source = self.tokenizer.decode(list(input_ids_array[idx]))
    ...
    def validation_epoch_end(self, outputs, prefix="val") -> Dict:
        ...
        if self.val_metric == 'bleu':
            metric_val = bleu = corpus_bleu(predictions, [[tgt] for tgt in targets])
    ...
    def calc_generative_metrics(self, preds, target) -> dict:
        if self.val_metric == 'bleu':
            ...
            return {'acc': self.calculate_acc(preds, target)}
...
def debug_eval():
    ckpt_path = Path('seq2seq/test_data/debug_t5/checkpoint_epoch14.ckpt')
    model = TranslationModule.load_from_checkpoint(
        str(ckpt_path), map_location='cpu',
        data_dir='/Users/pengcheng/Research/datasets/cfq/mcd2/t5_finetune',
        n_train=50
    )
    trainer = pl.Trainer(resume_from_checkpoint=str(ckpt_path))

    trainer.test(model, model.get_dataloader('train', batch_size=3), ckpt_path=ckpt_path)
